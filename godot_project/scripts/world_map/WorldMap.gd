...
func _populate_world_debug_panel() -> void:
    ...
    _create_world_debug_button(button_row_3, "Intake", "Toggle accepting new visitors.", _on_world_debug_toggle_visitor_intake_pressed)
    ...
    button_row_4.name = "WorldDebugButtonRow4"
    button_row_4.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    debug_holder.add_child(button_row_4)

    _create_world_debug_button(button_row_4, "Quest", "Create simple Slime Hunt quest.", _on_world_debug_create_slime_hunt_quest_pressed)
    _create_world_debug_button(button_row_4, "EncQ", "Spend 35g to encourage active quest.", _on_world_debug_encourage_active_quest_pressed)
    _create_world_debug_button(button_row_4, "ClearQ", "Clear active quest for testing.", _on_world_debug_clear_active_quest_pressed)

    var button_row_5 := HBoxContainer.new()
    button_row_5.name = "WorldDebugButtonRow5"
    button_row_5.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    debug_holder.add_child(button_row_5)

    _create_world_debug_button(button_row_5, "DiscT", "Debug-discover known threats.", _on_world_debug_discover_known_threats_pressed)
    _create_world_debug_button(button_row_5, "ResetT", "Reset known threat discovery data.", _on_world_debug_reset_known_threats_pressed)
...
func _on_world_debug_discover_known_threats_pressed() -> void:
    if GameState.has_method("debug_discover_known_threats"):
        GameState.debug_discover_known_threats()
    _refresh_world_sidebar_labels()

func _on_world_debug_reset_known_threats_pressed() -> void:
    if GameState.has_method("debug_reset_known_threats"):
        GameState.debug_reset_known_threats()
    _refresh_world_sidebar_labels()

func _on_world_debug_create_slime_hunt_quest_pressed() -> void:
    if GameState.has_method("debug_create_slime_hunt_quest"):
        GameState.debug_create_slime_hunt_quest()
    _refresh_world_sidebar_labels()


func _on_world_debug_encourage_active_quest_pressed() -> void:
    if GameState.has_method("encourage_active_quest_from_town"):
        GameState.encourage_active_quest_from_town()
    _refresh_world_sidebar_labels()

func _on_world_debug_clear_active_quest_pressed() -> void:
    if GameState.has_method("clear_active_quest_debug"):
        GameState.clear_active_quest_debug()
    _refresh_world_sidebar_labels()

func _on_world_debug_toggle_visitor_intake_pressed() -> void:
