...
const REGIONAL_ADVENTURER_BASE_CAP := 3
const GUILD_HALL_VISITOR_CAP_BONUS_PER_LEVEL := 1
...
var building_management_popup_canvas_layer: CanvasLayer = null
var building_management_popup_overlay: Control = null
var building_management_popup_panel: PanelContainer = null
var building_management_title_label: Label = null
var building_management_body: VBoxContainer = null
var building_management_scroll: ScrollContainer = null
var building_management_current_building: ColorRect = null
var building_management_current_building_type: String = ""
...
func _ready() -> void:
    ...
    _sync_regional_adventurer_cap_from_guild_hall()
    _create_sidebar_manager()
    _create_build_panel()
    _dock_build_panel_in_sidebar()
    _dock_building_details_in_sidebar()
    _create_building_management_popup_overlay()
...
func _process(_delta: float) -> void:
    ...
    _process_debug_max_trip_departures()
...
func spawn_placeholder_adventurer() -> void:
    if GameState.has_method("can_spawn_regional_visitor") and not GameState.can_spawn_regional_visitor():
        var block_reason: String = GameState.get_visitor_cap_warning_text() if GameState.has_method("get_visitor_cap_warning_text") else "Visitor spawn blocked."
        _update_build_status("Visitor spawn blocked: %s" % block_reason)
        show_town_floating_text("Spawn blocked: %s" % block_reason, town_entrance.global_position + Vector2(60, -20))
        if GameState.has_method("log_visitor_event"):
            GameState.log_visitor_event("spawn_blocked", "Visitor", block_reason)
        return
    ...
    var spawn_event: String = str(visitor_data.get("last_spawn_event", "new"))
    var visitor_name: String = str(visitor_data.get("display_name", "Visitor"))
    if spawn_event == "priority_return":
        show_town_floating_text("Priority return: %s" % visitor_name, entrance_position + Vector2(40, -20))
        _update_build_status("Priority return visitor arrived: %s" % visitor_name)
    elif spawn_event == "returning":
        show_town_floating_text("Returning visitor: %s" % visitor_name, entrance_position + Vector2(40, -20))
        _update_build_status("Returning visitor arrived: %s" % visitor_name)
    else:
        show_town_floating_text("New visitor registered: %s" % visitor_name, entrance_position + Vector2(40, -20))
        _update_build_status("New visitor registered at Guild Hall: %s" % visitor_name)
...
func _process_debug_max_trip_departures() -> void:
    ...
    for child in adventurers_container.get_children():
        ...
        var ai_node: Node = child.get("ai") if child.get("ai") != null else null
        var ai_state: String = ""
        if ai_node != null and ai_node.has_method("get_state"):
            ai_state = str(ai_node.get_state())

        if str(child.get("roster_role")) == "resident_placeholder":
            continue

        if ai_state == "MaxTripsReached" and GameState.has_method("get_visitor_departure_reason_for_adventurer"):
            var reason: String = GameState.get_visitor_departure_reason_for_adventurer(child)
            if reason == "debug_max_trips_complete":
                departing.append(child)

    for adventurer in departing:
        _release_visitor_from_region(adventurer, "debug_max_trips_complete")

func _release_visitor_from_region(adventurer: Node, reason: String) -> void:
    if adventurer == null:
        return
    ...
    adventurer.set("departure_reason_placeholder", reason)

    if GameState.has_method("release_visitor_to_pool_from_adventurer"):
        GameState.release_visitor_to_pool_from_adventurer(adventurer, reason)

    if adventurer.get_parent() != null:
        adventurer.get_parent().remove_child(adventurer)

    adventurer.queue_free()
    show_town_floating_text("%s left region: %s" % [departure_name, reason], departure_position)
    _update_build_status("%s left the local region. Reason: %s" % [departure_name, reason])
    _refresh_sidebar_debug()
...
func _process_visitor_departures_for_day(_day_number: int) -> void:
    ...
    for adventurer in departing:
        ...
        var reason: String = "visit_days_complete"
        if GameState.has_method("get_visitor_departure_reason_for_adventurer"):
            reason = str(GameState.get_visitor_departure_reason_for_adventurer(adventurer))
            if reason == "":
                reason = "visit_days_complete"

        _release_visitor_from_region(adventurer, reason)
...
func _on_building_clicked(building_id: String, building_node: Node = null) -> void:
    ...
    if building_node is ColorRect:
        call_deferred("_open_building_management_popup_from_node", building_node)
...
func force_close_transient_ui() -> void:
    _close_building_management_popup()
    if esc_main_menu_overlay != null:
        esc_main_menu_overlay.visible = false
    if esc_submenu_panel != null:
        esc_submenu_panel.visible = false
...
func _get_building_type_for_node(building: Node) -> String:
    if building == null:
        return ""

    if building.has_meta("building_type"):
        return str(building.get_meta("building_type"))

    if building.has_meta("building_id"):
        return str(building.get_meta("building_id"))

    if building.has_meta("type"):
        return str(building.get_meta("type"))

    var script_building_id: Variant = building.get("building_id")
    if script_building_id != null and str(script_building_id) != "":
        return str(script_building_id)

    return ""

func _get_building_capacity_for_popup(building_type: String, building: Node) -> int:
    if building != null and building.has_meta("capacity"):
        return int(building.get_meta("capacity"))

    if has_method("get_building_capacity"):
        return int(get_building_capacity(building_type))

    if BUILDING_CAPACITY.has(building_type):
        return int(BUILDING_CAPACITY[building_type])

    return 0

func _get_service_speed_multiplier_for_popup(building: Node) -> float:
    if building == null:
        return 1.0

    if building.has_meta("service_speed_multiplier"):
        return float(building.get_meta("service_speed_multiplier"))

    if building.has_meta("service_speed"):
        return float(building.get_meta("service_speed"))

    if building.has_meta("upgrade_level"):
        return 1.0 + float(building.get_meta("upgrade_level")) * 0.1

    return 1.0

func _get_worker_capacity_for_popup(building: Node) -> int:
    if building == null:
        return 0

    if building.has_meta("worker_capacity"):
        return int(building.get_meta("worker_capacity"))

    if building.has_meta("max_workers"):
        return int(building.get_meta("max_workers"))

    if building.has_meta("upgrade_level"):
        return maxi(1, 1 + int(building.get_meta("upgrade_level")))

    return 1


func _create_building_management_popup_overlay() -> void:
    if building_management_popup_overlay != null and is_instance_valid(building_management_popup_overlay):
        return

    building_management_popup_canvas_layer = CanvasLayer.new()
    building_management_popup_canvas_layer.name = "BuildingManagementPopupCanvasLayer"
    building_management_popup_canvas_layer.layer = 65
    building_management_popup_canvas_layer.process_mode = Node.PROCESS_MODE_ALWAYS
    add_child(building_management_popup_canvas_layer)

    building_management_popup_overlay = Control.new()
    building_management_popup_overlay.name = "BuildingManagementPopupOverlay"
    building_management_popup_overlay.visible = false
    building_management_popup_overlay.mouse_filter = Control.MOUSE_FILTER_STOP
    building_management_popup_overlay.process_mode = Node.PROCESS_MODE_ALWAYS
    building_management_popup_overlay.set_anchors_preset(Control.PRESET_FULL_RECT)
    building_management_popup_overlay.offset_left = 0.0
    building_management_popup_overlay.offset_top = 0.0
    building_management_popup_overlay.offset_right = 1600.0
    building_management_popup_overlay.offset_bottom = 720.0
    building_management_popup_overlay.size = Vector2(1600, 720)
    building_management_popup_overlay.z_index = 420
    building_management_popup_canvas_layer.add_child(building_management_popup_overlay)

    var blocker := ColorRect.new()
    blocker.name = "BuildingPopupSoftDim"
    blocker.color = Color(0, 0, 0, 0.36)
    blocker.mouse_filter = Control.MOUSE_FILTER_STOP
    blocker.process_mode = Node.PROCESS_MODE_ALWAYS
    blocker.set_anchors_preset(Control.PRESET_FULL_RECT)
    building_management_popup_overlay.add_child(blocker)

    building_management_popup_panel = PanelContainer.new()
    building_management_popup_panel.name = "BuildingManagementPopupPanel"
    building_management_popup_panel.offset_left = 260.0
    building_management_popup_panel.offset_top = 70.0
    building_management_popup_panel.offset_right = 1240.0
    building_management_popup_panel.offset_bottom = 660.0
    building_management_popup_panel.mouse_filter = Control.MOUSE_FILTER_STOP
    building_management_popup_panel.process_mode = Node.PROCESS_MODE_ALWAYS
    building_management_popup_panel.add_theme_stylebox_override("panel", _make_menu_panel_style())
    building_management_popup_overlay.add_child(building_management_popup_panel)

    var root_vbox := VBoxContainer.new()
    root_vbox.name = "BuildingManagementPopupRoot"
    root_vbox.add_theme_constant_override("separation", 8)
    root_vbox.process_mode = Node.PROCESS_MODE_ALWAYS
    building_management_popup_panel.add_child(root_vbox)

    var header_row := HBoxContainer.new()
    header_row.name = "BuildingManagementHeader"
    header_row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    header_row.process_mode = Node.PROCESS_MODE_ALWAYS
    root_vbox.add_child(header_row)

    building_management_title_label = Label.new()
    building_management_title_label.name = "BuildingManagementTitle"
    building_management_title_label.text = "Building"
    building_management_title_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    building_management_title_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
    building_management_title_label.process_mode = Node.PROCESS_MODE_ALWAYS
    header_row.add_child(building_management_title_label)

    var close_button := Button.new()
    close_button.name = "BuildingManagementCloseButton"
    close_button.text = "X"
    close_button.tooltip_text = "Close building management popup."
    close_button.focus_mode = Control.FOCUS_NONE
    close_button.custom_minimum_size = Vector2(42, 32)
    close_button.process_mode = Node.PROCESS_MODE_ALWAYS
    close_button.pressed.connect(_close_building_management_popup)
    header_row.add_child(close_button)

    building_management_scroll = ScrollContainer.new()
    building_management_scroll.name = "BuildingManagementScroll"
    building_management_scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    building_management_scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
    building_management_scroll.custom_minimum_size = Vector2(930, 500)
    building_management_scroll.process_mode = Node.PROCESS_MODE_ALWAYS
    root_vbox.add_child(building_management_scroll)

    building_management_body = VBoxContainer.new()
    building_management_body.name = "BuildingManagementBody"
    building_management_body.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    building_management_body.process_mode = Node.PROCESS_MODE_ALWAYS
    building_management_scroll.add_child(building_management_body)

func _open_building_management_popup_from_node(building: Node) -> void:
    if building is ColorRect:
        _open_building_management_popup(building as ColorRect)

func _open_building_management_popup(building: ColorRect) -> void:
    if building == null or not is_instance_valid(building):
        return

    if building_management_popup_overlay == null:
        _create_building_management_popup_overlay()

    if building_management_popup_overlay == null:
        push_warning("Building management popup overlay could not be created.")
        return

    building_management_current_building = building
    building_management_current_building_type = _get_building_type_for_node(building)
    building_management_popup_overlay.visible = true
    building_management_popup_overlay.show()

    if building_management_popup_panel != null:
        building_management_popup_panel.visible = true
        building_management_popup_panel.show()

    _refresh_building_management_popup()

func _close_building_management_popup() -> void:
    if building_management_popup_overlay != null:
        building_management_popup_overlay.visible = false
    building_management_current_building = null
    building_management_current_building_type = ""

func _refresh_building_management_popup() -> void:
    if building_management_body == null:
        return

    for child in building_management_body.get_children():
        child.queue_free()

    if building_management_current_building == null or not is_instance_valid(building_management_current_building):
        _close_building_management_popup()
        return

    var building_type: String = building_management_current_building_type
    if building_type == "":
        building_type = _get_building_type_for_node(building_management_current_building)

    if building_management_title_label != null:
        building_management_title_label.text = "%s Management" % _get_building_display_name(building_type)

    _add_building_popup_summary_section(building_type, building_management_current_building)

    match building_type:
        "guild_hall":
            _populate_guild_hall_management_popup(building_management_current_building)
        "general_store":
            _populate_general_store_management_popup(building_management_current_building)
        "inn":
            _populate_inn_management_popup(building_management_current_building)
        _:
            _populate_generic_building_management_popup(building_type, building_management_current_building)

func _add_building_popup_summary_section(building_type: String, building: ColorRect) -> void:
    var label := Label.new()
    label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
    label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    label.text = "BUILDING SUMMARY\nType: %s\nLevel: %d\nCapacity: %d\nService Speed: %.2fx\nWorkers: %d/%d\n\nThis popup is the new expandable building-management foundation. Sidebar details remain as fallback/debug for now." % [
        _get_building_display_name(building_type),
        get_upgrade_level_for_building(building),
        _get_building_capacity_for_popup(building_type, building),
        _get_service_speed_multiplier_for_popup(building),
        int(building.get_meta("worker_count", 0)),
        _get_worker_capacity_for_popup(building)
    ]
    building_management_body.add_child(label)

func _add_popup_separator() -> void:
    var separator := HSeparator.new()
    separator.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    building_management_body.add_child(separator)

func _add_popup_text(text: String) -> Label:
    var label := Label.new()
    label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
    label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    label.text = text
...
func _add_popup_button(text: String, tooltip: String, callback: Callable) -> Button:
    var button := Button.new()
    button.text = text
    button.tooltip_text = tooltip
    button.focus_mode = Control.FOCUS_NONE
    ...
    button.pressed.connect(callback)
    building_management_body.add_child(button)
    return button

func _populate_guild_hall_management_popup(building: ColorRect) -> void:
    _add_popup_separator()
    _add_popup_text("GUILD HALL / VISITOR INTAKE\n%s" % get_guild_hall_visitor_cap_summary(building))
    _add_popup_button("Toggle Visitor Intake", "Open or close visitor intake.", func() -> void:
        toggle_visitor_intake_from_guild_hall()
        _refresh_building_management_popup()
    )

    _add_popup_separator()
    _add_popup_text(GameState.get_quest_board_status_text() if GameState.has_method("get_quest_board_status_text") else "Quest board unavailable.")
    _add_popup_button("Post Slime Hunt Quest", "Create the current simple Slime Hunt objective.", func() -> void:
        var message: String = GameState.debug_create_slime_hunt_quest() if GameState.has_method("debug_create_slime_hunt_quest") else "Quest creation unavailable."
        _update_build_status(message)
        show_town_floating_text(message, town_entrance.global_position + Vector2(80, -28))
        _refresh_sidebar_debug()
        _refresh_building_management_popup()
    )
    _add_popup_button("Encourage Questing - 35g", "Spend town funds to increase adventurer interest in the active quest.", func() -> void:
        var message: String = GameState.encourage_active_quest_from_town() if GameState.has_method("encourage_active_quest_from_town") else "Quest encouragement unavailable."
        _update_build_status(message)
        show_town_floating_text(message, town_entrance.global_position + Vector2(80, -28))
        _refresh_sidebar_debug()
        _refresh_building_management_popup()
    ...
    _add_popup_separator()
    _add_popup_text(GameState.get_known_threats_status_text() if GameState.has_method("get_known_threats_status_text") else "Known threats unavailable.")

    _add_popup_separator()
    _add_popup_text("GUILD REGISTRY\nKnown adventurer actions are now shown per row as placeholders.\nPriority return remains one active requested adventurer at a time.")
    _populate_guild_hall_registry_rows()

func _populate_guild_hall_registry_rows() -> void:
    var entries: Array[Dictionary] = []
    if GameState.has_method("get_known_adventurer_registry_entries"):
        entries = GameState.get_known_adventurer_registry_entries(10)

    if entries.is_empty():
        _add_popup_text("No known adventurers registered yet. Spawn visitors to populate the Guild Registry.")
        return

    for entry in entries:
        var row_panel := PanelContainer.new()
        row_panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
        row_panel.add_theme_stylebox_override("panel", _make_menu_panel_style())
        building_management_body.add_child(row_panel)
        ...
        summary_label.text = "%s | Visits:%d | Status:%s | Last:%s\nFavorite:%s | Priority:%s | Contract:%s | House:%s" % [
            str(entry.get("display_name", "Visitor")),
            int(entry.get("total_visits", 0)),
            str(entry.get("status", "unknown")),
            str(entry.get("departure_reason", "none")),
            "Yes" if bool(entry.get("favorite_placeholder", false)) else "No",
            "Yes" if bool(entry.get("priority_invite_placeholder", false)) else "No",
            str(entry.get("contract_status", "no_contract")),
            str(entry.get("house_request_status", "no_house_request"))
        ]
        row_vbox.add_child(summary_label)

        var button_row := HBoxContainer.new()
        button_row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
        row_vbox.add_child(button_row)

        var visitor_id: int = int(entry.get("visitor_id", -1))

        var fav_button := Button.new()
        fav_button.text = "Favorite"
        fav_button.focus_mode = Control.FOCUS_NONE
        fav_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
        fav_button.pressed.connect(func() -> void:
            _popup_toggle_favorite_for_visitor(visitor_id)
        )
        button_row.add_child(fav_button)

        var priority_button := Button.new()
        priority_button.text = "Request Return"
        priority_button.tooltip_text = "Marks this adventurer for priority return. Only one can be marked at a time."
        priority_button.focus_mode = Control.FOCUS_NONE
        priority_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
        priority_button.pressed.connect(func() -> void:
            _popup_mark_priority_return_for_visitor(visitor_id)
        )
        button_row.add_child(priority_button)

        var contract_button := Button.new()
        contract_button.text = "Contract"
        ...
        contract_button.pressed.connect(func() -> void:
            _popup_contract_visitor(visitor_id)
        )
        button_row.add_child(contract_button)

func _popup_toggle_favorite_for_visitor(visitor_id: int) -> void:
    var message: String = "Favorite action unavailable."
    if GameState.has_method("toggle_known_adventurer_favorite_by_id"):
        message = str(GameState.toggle_known_adventurer_favorite_by_id(visitor_id))

    _update_build_status(message)
    show_town_floating_text(message, town_entrance.global_position + Vector2(70, -28))
    _refresh_sidebar_debug()
    _refresh_building_management_popup()

func _popup_mark_priority_return_for_visitor(visitor_id: int) -> void:
    var message: String = "Priority return action unavailable."
    if GameState.has_method("mark_priority_return_by_id"):
        message = str(GameState.mark_priority_return_by_id(visitor_id))
...
func _popup_contract_visitor(visitor_id: int) -> void:
    var message: String = "Contract action unavailable."
    if GameState.has_method("contract_known_adventurer_by_id"):
        message = str(GameState.contract_known_adventurer_by_id(visitor_id))
...
func _populate_general_store_management_popup(_building: ColorRect) -> void:
    _add_popup_separator()
    _add_popup_text("GENERAL STORE MANAGEMENT PLACEHOLDER\n\nFuture controls:\n- Toggle which goods the store buys.\n- Toggle which goods the store sells.\n- Set stock targets.\n- Set price markup.\n- Review adventurer purchases.\n- Connect to crafted/general goods economy.\n\nCurrent policy:\n%s" % GameState.get_general_store_buy_policy_text())
    _add_popup_button("Toggle Slime Gel Buying", "Toggle whether the General Store buys Slime Gel.", func() -> void:
        GameState.toggle_general_store_buys_slime_gel()
        _refresh_building_management_popup()
    )

func _populate_inn_management_popup(_building: ColorRect) -> void:
    _add_popup_separator()
    _add_popup_text("INN MANAGEMENT PLACEHOLDER\n\nFuture controls:\n- Room pricing.\n- Rest quality.\n- Meal/buff package options.\n- Worker assignment.\n- Guest history.\n- Adventurer recovery preferences.\n\nCurrent prototype use: adventurers rest/sleep here when tired, injured, or during night behavior.")

func _populate_generic_building_management_popup(building_type: String, _building: ColorRect) -> void:
    _add_popup_separator()
    _add_popup_text("%s MANAGEMENT PLACEHOLDER\n\nThis building does not have a custom management screen yet.\n\nFuture building popups should support specialized controls for shops, crafting, restaurants, production, housing, decorations, and service buildings." % _get_building_display_name(building_type))
...
func _populate_debug_placeholder() -> void:
    ...
    _create_sidebar_debug_button(button_row_4, "Intake", "Toggle accepting new visitors.", _on_sidebar_debug_toggle_visitor_intake_pressed)
    ...
    var button_row_5 := HBoxContainer.new()
    button_row_5.name = "SidebarDebugButtonRow5"
    button_row_5.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    debug_placeholder_holder.add_child(button_row_5)

    _create_sidebar_debug_button(button_row_5, "Quest", "Create a simple Slime Hunt quest.", _on_sidebar_debug_create_slime_hunt_quest_pressed)
    _create_sidebar_debug_button(button_row_5, "EncQ", "Spend 35g to encourage active quest.", _on_sidebar_debug_encourage_active_quest_pressed)
    _create_sidebar_debug_button(button_row_5, "ClearQ", "Clear active quest for testing.", _on_sidebar_debug_clear_active_quest_pressed)

    var button_row_6 := HBoxContainer.new()
    button_row_6.name = "SidebarDebugButtonRow6"
    button_row_6.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    debug_placeholder_holder.add_child(button_row_6)

    _create_sidebar_debug_button(button_row_6, "DiscT", "Debug-discover known threats.", _on_sidebar_debug_discover_known_threats_pressed)
    _create_sidebar_debug_button(button_row_6, "ResetT", "Reset known threat discovery data.", _on_sidebar_debug_reset_known_threats_pressed)
...
func _on_sidebar_debug_discover_known_threats_pressed() -> void:
    var message: String = "Known threat discovery unavailable."
    if GameState.has_method("debug_discover_known_threats"):
        message = str(GameState.debug_discover_known_threats())

    show_town_floating_text(message, town_entrance.global_position + Vector2(80, -28))
    _update_build_status(message)
    _refresh_sidebar_debug()
    if building_management_popup_overlay != null and building_management_popup_overlay.visible:
        _refresh_building_management_popup()

func _on_sidebar_debug_reset_known_threats_pressed() -> void:
    var message: String = "Known threat reset unavailable."
    if GameState.has_method("debug_reset_known_threats"):
        message = str(GameState.debug_reset_known_threats())
...
func _on_sidebar_debug_create_slime_hunt_quest_pressed() -> void:
    var message: String = "Quest creation unavailable."
    if GameState.has_method("debug_create_slime_hunt_quest"):
        message = str(GameState.debug_create_slime_hunt_quest())
...
func _on_sidebar_debug_encourage_active_quest_pressed() -> void:
    var message: String = "Quest encouragement unavailable."
    if GameState.has_method("encourage_active_quest_from_town"):
        message = str(GameState.encourage_active_quest_from_town())
...
func _on_sidebar_debug_clear_active_quest_pressed() -> void:
    var message: String = "Quest clear unavailable."
    if GameState.has_method("clear_active_quest_debug"):
        message = str(GameState.clear_active_quest_debug())
...
func _on_sidebar_debug_toggle_visitor_intake_pressed() -> void:
    if GameState.has_method("toggle_visitor_intake"):
        GameState.toggle_visitor_intake()
    show_town_floating_text("Intake: %s" % GameState.get_visitor_intake_policy_text(), town_entrance.global_position + Vector2(60, -28))
    _refresh_sidebar_debug()
...
func _get_sidebar_debug_text() -> String:
    return "DUNGEON FRONTIER DEBUG\n\n-- State --\nView: %s\nTime: Day %d - %s (%ds left)\nVillage Funds: %d\nInventory: Potions %d | Slime Gel %d\nIn-Town Adventurers: %d\nWorld Travelers: %d\nReturned Travelers: %d\nTraveler Status: %s\nReturned: %s\nVisitor Compact: %s\nVisitor Pool: %s\nVisitor Warning: %s\nDeparture: %s\nPriority: %s\nContracts: %s\nHousing: %s\n%s\n\n-- Quest --\n%s\n%s\n%s\n%s\n%s\n\n-- Threat --\nSlime Nest: %s | Growth %d | Level %d | %s\nSlime Spawns: %s\nNight Quests: %s\nNight Danger: %s\nRetreat E<=40\nGeneral Store: %s\n\n-- Save --\nAutosave: daily only\nAutosave Clear: %s\nManual slots are controlled from Save tab." % [
        ...
        GameState.get_visitor_compact_status_text() if GameState.has_method("get_visitor_compact_status_text") else "Unavailable",
        GameState.get_visitor_population_status_text() if GameState.has_method("get_visitor_population_status_text") else "Unavailable",
        GameState.get_visitor_cap_warning_text() if GameState.has_method("get_visitor_cap_warning_text") else "Unavailable",
        GameState.get_last_departure_reason_text() if GameState.has_method("get_last_departure_reason_text") else "Unavailable",
        GameState.get_priority_return_candidate_text() if GameState.has_method("get_priority_return_candidate_text") else "Unavailable",
        GameState.get_resident_contract_status_text() if GameState.has_method("get_resident_contract_status_text") else "Unavailable",
        GameState.get_house_request_placeholder_text() if GameState.has_method("get_house_request_placeholder_text") else "Unavailable",
        GameState.get_visitor_event_log_text(4) if GameState.has_method("get_visitor_event_log_text") else "Visitor Log unavailable.",
        GameState.get_known_threats_status_text() if GameState.has_method("get_known_threats_status_text") else "Known threats unavailable.",
        GameState.get_quest_board_status_text() if GameState.has_method("get_quest_board_status_text") else "Quest board unavailable.",
        GameState.get_quest_reward_economy_status_text() if GameState.has_method("get_quest_reward_economy_status_text") else "Quest reward economy unavailable.",
        GameState.get_quest_encouragement_economy_status_text() if GameState.has_method("get_quest_encouragement_economy_status_text") else "Quest encouragement economy unavailable.",
        GameState.get_quest_reward_spending_loop_status_text() if GameState.has_method("get_quest_reward_spending_loop_status_text") else "Quest reward spending unavailable.",
        GameState.get_adventurer_quest_reward_wallet_status_text() if GameState.has_method("get_adventurer_quest_reward_wallet_status_text") else "Quest reward wallets unavailable.",
...
func select_building(building: ColorRect) -> void:
    ...
    _open_building_management_popup_from_node(building)
    _update_selected_outline()
...
func _on_demolish_selected_button_pressed() -> void:
    ...
    if building_management_current_building == building_to_remove:
        _close_building_management_popup()
...
func _sync_regional_adventurer_cap_from_guild_hall() -> void:
    if not GameState.has_method("set_regional_adventurer_cap"):
        return

    GameState.set_regional_adventurer_cap(get_effective_regional_adventurer_cap())

func get_effective_regional_adventurer_cap() -> int:
    return REGIONAL_ADVENTURER_BASE_CAP + get_guild_hall_cap_bonus_placeholder()

func get_guild_hall_cap_bonus_placeholder() -> int:
    return get_highest_guild_hall_upgrade_level() * GUILD_HALL_VISITOR_CAP_BONUS_PER_LEVEL

func get_highest_guild_hall_upgrade_level() -> int:
    var highest_level: int = 0

    for building in _get_placed_buildings_of_type("guild_hall"):
        if building is ColorRect:
            highest_level = maxi(highest_level, get_upgrade_level_for_building(building as ColorRect))

    return highest_level

func get_guild_hall_visitor_cap_summary(building_node: Node) -> String:
    if not building_node is ColorRect:
        return "Guild Hall cap data unavailable."

    var building := building_node as ColorRect
    var level: int = get_upgrade_level_for_building(building)
    var this_bonus: int = level * GUILD_HALL_VISITOR_CAP_BONUS_PER_LEVEL
    var highest_level: int = get_highest_guild_hall_upgrade_level()
    var active_count: int = GameState.get_active_regional_adventurer_count()
    var current_cap: int = GameState.get_regional_adventurer_cap()
    var open_slots: int = GameState.get_regional_adventurer_open_slots()
    var available_pool: int = GameState.get_available_visitor_pool_count()
    var pool_total: int = GameState.visitor_pool.size()
    var status_text: String = GameState.get_visitor_cap_warning_text()
    var intake_text: String = GameState.get_visitor_intake_policy_text()

    return "Regional Adventurer Cap: %d/%d active\\nOpen Slots: %d\\nVisitor Pool: %d available / %d total\\nIntake: %s\\nThis Guild Hall Lv.%d bonus: +%d\\nHighest Guild Hall Lv.%d controls placeholder cap bonus: +%d\\nStatus: %s" % [
        active_count,
        current_cap,
        open_slots,
        available_pool,
...
func get_guild_hall_registry_summary(_building_node: Node) -> String:
    var known_count: int = GameState.get_known_adventurer_count() if GameState.has_method("get_known_adventurer_count") else 0
    var registry_text: String = GameState.get_known_adventurer_registry_text(6) if GameState.has_method("get_known_adventurer_registry_text") else "Known registry unavailable."
    return "Known Adventurers: %d\n%s" % [
        known_count,
        registry_text
    ]

func contract_first_eligible_favorite_from_guild_hall() -> void:
    var message: String = "Contract unavailable."
    if GameState.has_method("contract_first_eligible_favorite_placeholder"):
        message = str(GameState.contract_first_eligible_favorite_placeholder())

    _update_build_status(message)
    _refresh_sidebar_debug()
    show_town_floating_text(message, town_entrance.global_position + Vector2(70, -28))

func toggle_favorite_known_adventurer_from_guild_hall() -> void:
    var message: String = "Favorite toggle unavailable."
    if GameState.has_method("toggle_first_known_adventurer_favorite_placeholder"):
        message = str(GameState.toggle_first_known_adventurer_favorite_placeholder())

    _update_build_status(message)
    _refresh_sidebar_debug()
    show_town_floating_text(message, town_entrance.global_position + Vector2(70, -28))

func mark_priority_return_from_guild_hall() -> void:
    var message: String = "Priority return unavailable."
    if GameState.has_method("mark_priority_return_placeholder"):
        message = str(GameState.mark_priority_return_placeholder())

    _update_build_status(message)
    _refresh_sidebar_debug()
    show_town_floating_text(message, town_entrance.global_position + Vector2(70, -28))

func toggle_visitor_intake_from_guild_hall() -> void:
    if GameState.has_method("toggle_visitor_intake"):
        GameState.toggle_visitor_intake()
        _update_build_status("Visitor intake: %s" % GameState.get_visitor_intake_policy_text())
        _refresh_sidebar_debug()
